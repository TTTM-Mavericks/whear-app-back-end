...
public class FollowServiceImpl implements FollowService {
    ...
    public FollowResponse userFollowAnotherUser(FollowRequest followRequest) throws CustomException {
        ...
        User firstUser = Optional.ofNullable(userService.getUserEntityByUsername(followRequest.getFirstUsername()))
                .orElseThrow(() -> handleUserNotFound(followRequest.getFirstUsername()));

        User secondUser = Optional.ofNullable(userService.getUserEntityByUsername(followRequest.getSecondUsername()))
        ...
        FollowerKey followerKey = Optional.of(FollowerKey
                ...
                .orElseThrow(() -> new RuntimeException("Failed to create FollowerKey. Because Two Username are the same"));
        if(followerRepository.findFollowerByFollowerIdAndFollowingId(followRequest.getFirstUsername(), followRequest.getSecondUsername()) != null)
        {
            logger.error(ConstantMessage.USERNAME_IS_EXIST.getMessage() + ": " + followRequest.getFirstUsername() + " and " + followRequest.getSecondUsername());
            throw new CustomException(ConstantMessage.USERNAME_IS_EXIST.getMessage() + " : " + followRequest.getFirstUsername() + " and " + followRequest.getSecondUsername());
        }
        Follower follower = new Follower();
        follower.setFollowerKey(followerKey);
        followerRepository.save(follower);
        return new FollowResponse(userService.convertToUserResponse(firstUser), userService.convertToUserResponse(secondUser));
    }

    @Override
    public List<UserResponse> getAllFollowerUser(String username) throws CustomException // List All User Follower the User
    {
        ...
        List<UserResponse> userResponseList = followerRepository.findAllFollowingUserByUsername(username)
                .stream()
                .map(following -> userService.convertToUserResponse(following.getFollowerKey().getFollowerUserID()))
                .toList();
        return userResponseList;
    }

    @Override
    public List<UserResponse> getAllFollowingUser(String username) throws CustomException // List All User, User Following
    {
        Optional.of(username)
                .filter(userId -> !userId.isEmpty() && !userId.isBlank())
                .orElseThrow(() -> handleInvalidUsername(username));
        ...
        List<UserResponse> userResponseList = followerRepository.findAllFollowerUserByUsername(username)
                .stream()
                .map(follower -> userService.convertToUserResponse(follower.getFollowerKey().getFollowingUserID()))
                .toList();
        return userResponseList;
    }
