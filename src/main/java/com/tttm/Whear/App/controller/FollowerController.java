...
public class FollowerController {
    ...
    public ObjectNode userFollowAnotherUser(@RequestBody FollowRequest followRequest) throws CustomException {
        ObjectMapper objectMapper = new ObjectMapper();
        try {
            ObjectNode respon = objectMapper.createObjectNode();
            respon.put("success", 200);
            respon.put("message", "User Follow Another User Successfully");
            respon.set("data", objectMapper.valueToTree(followService.userFollowAnotherUser(followRequest)));
            return respon;
        } catch (Exception ex){
            ObjectNode respon = objectMapper.createObjectNode();
            respon.put("error", -1);
    ...
    public ObjectNode getAllFollowerUser(@RequestParam("username") String username) throws CustomException {
        ObjectMapper objectMapper = new ObjectMapper();
        try {
            ObjectNode respon = objectMapper.createObjectNode();
            respon.put("success", 200);
    ...
    public ObjectNode getAllFollowingUser(@RequestParam("username") String username) throws CustomException {
        ObjectMapper objectMapper = new ObjectMapper();
        try {
            ObjectNode respon = objectMapper.createObjectNode();
            respon.put("success", 200);
